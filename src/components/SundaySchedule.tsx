...
// Get calendar days for a month (including padding from prev/next months)
...
export default function SundaySchedule({ token, isSuperUser, departments }: SundayScheduleProps) {
  ...
  const handleSundayClick = (date: Date) => {
    ...
    if (date.getMonth() !== displayedMonth.getMonth()) return;
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="p-4 space-y-4">
              <p className="text-sm text-gray-600">
                Select a user who will have permission to edit the Sunday schedule.
              </p>
              <select
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowConfigModal(false)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div key={index} className="flex items-center gap-2 p-2 bg-gray-50 rounded-md">
                    {editingItemIndex === index ? (
                      <>
                        ...
                        />
                        <button onClick={() => renameItem(editingConfig, index)} className="px-2 py-1 bg-green-600 text-white rounded text-xs">Save</button>
                        <button onClick={() => { setEditingItemIndex(null); setEditingItemValue(''); }} className="px-2 py-1 bg-gray-400 text-white rounded text-xs">Cancel</button>
                      </>
                    ) : (
                      <>
                        <span className="flex-1 text-sm text-gray-700">{item}</span>
                        <button onClick={() => moveItem(editingConfig, index, 'up')} disabled={index === 0} className="p-1 text-gray-500 hover:text-gray-700 disabled:opacity-30">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
                        </button>
                        <button onClick={() => moveItem(editingConfig, index, 'down')} disabled={index === config[editingConfig].length - 1} className="p-1 text-gray-500 hover:text-gray-700 disabled:opacity-30">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                        </button>
                        <button onClick={() => { setEditingItemIndex(index); setEditingItemValue(item); }} className="p-1 text-blue-600 hover:text-blue-800">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        </button>
                        <button onClick={() => { if (confirm(`Delete "${item}"?`)) deleteItem(editingConfig, index); }} className="p-1 text-red-600 hover:text-red-800">
            ...
            <div className="p-4 border-t">
              <button onClick={() => setShowConfigModal(false)} className="w-full px-4 py-2 bg-gray-200 text-gray-700 rounded-md text-sm hover:bg-gray-300">Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedSunday && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setSelectedSunday(null)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-gray-800">{formatDate(selectedSunday)}</h3>
                {scheduleAdminName && (
                  <p className="text-sm text-gray-500">Admin: {scheduleAdminName}</p>
                )}
              </div>
              <div className="flex items-center gap-2">
                {canEdit && (
                  <>
                    <button
                      onClick={() => { setEditingConfig('phases'); setShowConfigModal(true); }}
                      className="px-3 py-1 text-sm bg-gray-100 rounded-md hover:bg-gray-200"
                    >
                      Phases
                    </button>
                    <button
                      onClick={() => { setEditingConfig('departments'); setShowConfigModal(true); }}
                      className="px-3 py-1 text-sm bg-gray-100 rounded-md hover:bg-gray-200"
                    >
                      Depts
                    </button>
                ...
                <button onClick={() => setSelectedSunday(null)} className="px-4 py-2 text-blue-600 hover:text-blue-800 font-medium">
                  Done
                </button>
              </div>
            </div>
                  ...
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      {config.departments.map((dept) => (
                        <th key={dept} className="px-2 py-2 text-center text-xs font-semibold text-gray-700 border-b whitespace-nowrap min-w-[100px]">
                          {dept}
                        </th>
                      ))}
                    </tr>
                      ...
                      <tr key={phase} className={phaseIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        {config.departments.map((dept) => {
                          ...
                          return (
                            <td
                              key={`${phase}-${dept}`}
                              className={`px-2 py-1 text-xs border-r border-b ${canEdit ? 'cursor-pointer hover:bg-blue-50' : ''} ${isEditing ? 'bg-blue-100 relative' : ''}`}
                                ...
                                <div className="relative" onClick={(e) => e.stopPropagation()}>
                                  <div className="min-h-[40px] p-1 bg-white border border-blue-400 rounded text-gray-900">
                                    <div className="flex flex-wrap gap-1 mb-1">
                                      {selectedUsers.map((u) => (
                                        <span key={u.id} className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-100 text-blue-800 rounded text-xs">
                                          {u.name}
                                          <button type="button" onClick={() => toggleUserSelection({ id: u.id, name: u.name, email: '' })} className="text-blue-600 hover:text-blue-800">&times;</button>
                                        </span>
                                      ))}
                                    </div>
                                    <button type="button" onClick={() => setShowUserDropdown(!showUserDropdown)} className="text-xs text-blue-600 hover:text-blue-800">+ Add user</button>
                                  </div>
                                  {showUserDropdown && (
                                        ...
                                        users.map((user) => {
                                          ...
                                          return (
                                            <button
                                              ...
                                              className={`w-full text-left px-3 py-2 text-xs hover:bg-gray-100 flex items-center gap-2 ${isSelected ? 'bg-blue-50 text-blue-800' : 'text-gray-700'}`}
                                            >
                                  ...
                                  <div className="flex gap-1 mt-1">
                                    <button type="button" onClick={saveCell} className="px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">Save</button>
                                    <button type="button" onClick={cancelEditing} className="px-2 py-1 text-xs bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
                                  </div>
                                </div>
                              ) : (
            ...
            {saving && (
              <div className="p-2 text-sm text-blue-600 flex items-center justify-center gap-2 border-t">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {isSuperUser && (
                <button onClick={openAdminModal} className="text-sm text-blue-600 hover:text-blue-800 underline">
                  Change
                </button>
              )}
            </div>
          ...
          {/* Month Navigation */}
          <div className="flex items-center gap-2">
            <button onClick={goToPreviousMonth} className="p-2 rounded-md border border-gray-300 hover:bg-gray-50">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>

            <div className="px-4 py-2 bg-blue-50 rounded-md min-w-[180px] text-center">
              <span className="font-medium text-blue-800">{monthYearString}</span>
            </div>

            <button onClick={goToNextMonth} className="p-2 rounded-md border border-gray-300 hover:bg-gray-50">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            </button>

            <button onClick={goToCurrentMonth} className="px-3 py-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200">
              Today
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-7 bg-gray-100">
          {daysOfWeek.map((day) => (
            <div key={day} className={`py-3 text-center text-sm font-semibold ${day === 'Sun' ? 'text-blue-600' : 'text-gray-600'}`}>
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          {calendarDays.map((date, index) => {
            ...
            const isSelected = selectedSunday && isSameDay(date, selectedSunday);

            return (
              <div
                key={index}
                onClick={() => handleSundayClick(date)}
                className={`
                  min-h-[80px] p-2 border-b border-r border-gray-100 transition-colors
                  ${isSundayDate && isCurrentMonth ? 'cursor-pointer hover:bg-blue-50 bg-blue-50/30' : ''}
              ...
              >
                <div className="flex flex-col items-center">
                  <div className={`
                    ...
                    ${isTodayDate ? 'bg-green-500 text-white font-semibold' : ''}
                    ${isSelected && !isTodayDate ? 'bg-blue-600 text-white font-semibold' : ''}
                    ${isSundayDate && isCurrentMonth && !isTodayDate && !isSelected ? 'border-2 border-blue-500 text-blue-600 font-semibold' : ''}
                    ${!isCurrentMonth ? 'text-gray-300' : ''}
                    ${!isSundayDate && isCurrentMonth && !isTodayDate ? 'text-gray-700' : ''}
                  `}>
                    {date.getDate()}
                  </div>
        ...
        {/* Legend */}
        <div className="p-4 bg-gray-50 flex flex-wrap gap-4 justify-center text-sm">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-blue-500"></div>
            <span className="text-gray-600">Sunday (tap to view schedule)</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        <ul className="text-xs text-gray-600 space-y-1">
          <li>• Click on any Sunday to view or edit the schedule for that day</li>
