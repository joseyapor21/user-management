...
// Check if user can manage this department (SuperUser or department admin)
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const memberIds = (department.memberIds || []).map((id: string) => {
      try {
    ...
    const normalizedMembers = members.map((u) => ({
      ...u,
      id: u._id.toString(),
    }));

    return NextResponse.json({ success: true, data: normalizedMembers });
  } catch (error) {
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // Add to memberIds (using $addToSet to avoid duplicates)
